...
public class DebtorAdapter extends RecyclerView.Adapter<DebtorAdapter.DebtorViewHolder> {
    ...
    public interface OnDebtorClickListener {
        void onDebtorClick(Debtor debtor);
    }

    private List<Debtor> debtors = new ArrayList<>();
    private final OnDebtorClickListener listener;
    private final Context context;

    // Colors
    private static final int COLOR_FULLY_PAID   = Color.parseColor("#2E7D32"); // dark green
    private static final int COLOR_PARTIAL       = Color.parseColor("#C62828"); // dark red
    ...
    public DebtorAdapter(Context context, OnDebtorClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull DebtorViewHolder holder, int position) {
        ...
        holder.bind(debtor);
        holder.itemView.setOnClickListener(v -> listener.onDebtorClick(debtor));
    }
    ...
    class DebtorViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(Debtor debtor) {
            ...
            if (debtor.isFullyPaid()) {
                ...
                tvInitial.setBackgroundTintList(android.content.res.ColorStateList.valueOf(COLOR_FULLY_PAID));
            } else {
                // Still has balance — RED
                tvName.setTextColor(COLOR_PARTIAL);
                double remaining = debtor.getRemainingBalance();
                tvBalance.setText(String.format("₱%.2f natitira", remaining));
                ...
                tvInitial.setBackgroundTintList(android.content.res.ColorStateList.valueOf(COLOR_PARTIAL));
            }
        }
    }
}
